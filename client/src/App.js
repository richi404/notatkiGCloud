...
function App() {
  ...
  const {data, addNote, deleteNote, isLoading, isError} = useNotes(1);
  ...
  return (
    <div className="App" style={{ marginTop: "25px" }}>
      ...
      {showInput ?  <AddNoteComponent addNote={addNote} setShowInput={setShowInput}/> : <Button text={"Add"} onClick={() => { setShowInput(true); }} />}
    </div>
  );
}
